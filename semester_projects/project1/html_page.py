...
<form method=POST action="">
    ...
    <table align="center" style="text-align: center">
        ...
        <td>
            ...
            <input type="checkbox" name="shops" value="Lamoda" checked>Lamoda.com.com
...
class Server(AllInfoBd):
    def __init__(self):
        ...
        self.list_sizes = ["None"] + [str(i) for i in range(35, 49)] + ["XS", "S", "M", "L", "XL"]
        self.days = ["0" + str(i) for i in range(1, 10)] + [str(i) for i in range(10, 32)]
        self.month = ["0" + str(i) for i in range(1, 10)] + ["10", "11", "12"]


    def combine_shops(self,type_request ,shops, request, type_sort, gender, size, num):

        ID = self.add_all_info(type_request, request, type_sort, gender, size, num)
        all_info = []
        ...
        for shop in shops:
            ...
            if shop == "Asos":
                ...
                if size.isdigit():
                    a_size = "eu " + size
                all_info.append(my_parsers.Asos(request, type_sort, gender, a_size, num))

            elif shop == "Lamoda":
                all_info.append(my_parsers.Lamoda(request, type_sort, gender, size, num))
                ...
                if information["price"] != "NaN":
                    ...
                    for name, value in information.items():
                        ...
                        if name == "price":
                            ...
                            price = convert(str(value))

                        elif name == "sale price":
                            sale_price = convert(str(value))
    ...
    def monitoring(self,type_request, shops, request, type_sort, gender, size, num):
        info, ID = self.combine_shops(type_request,shops, request, type_sort, gender, size, int(num))
        self.info_to_form(info, ID)
        return "Success"
    ...
    def application(self, environ, start_response):
        ...
        if environ.get('PATH_INFO', '').lstrip('/') == '':
            ...
            if form["submit"].value == "Search":
                ...
                info, ID = self.combine_shops("client_request",shops, request, type_sort, gender, size, int(num))
